...
class RobotController(Node):
    ...
    def _range_msg_callback(self, msg: Range):
        ...
        # The sensor only drives transitions between go_forward and
        # avoid_obstacle_back.  The avoid_obstacle_right state is owned
        # entirely by the control loop — the sensor never touches it.
        if self._patrol_state == "go_forward" and msg.range <= self._avoid_range:
            self._patrol_state = "avoid_obstacle_back"
            self.get_logger().info("Obstacle too close - avoiding!")
        elif self._patrol_state == "avoid_obstacle_back" and msg.range > self._avoid_range:
            self._turn_counter = 0
    ...
    def _control_loop(self):
        ...
        # PATROL
        cmd = Twist()
        if self._patrol_state == "go_forward":
            ...
            if self._turn_counter >= self._turn_iterations:
                self._turn_counter = 0
                self._patrol_state = "go_forward"
                self.get_logger().info("Turn complete. Going forward ...")
            else:
                self.get_logger().info(
                    f"Turning right ({self._turn_counter}/{self._turn_iterations}) ..."
                )
        else:
